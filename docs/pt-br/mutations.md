...
A única forma de altearr um estado numa store do Vuex é cometendo uma mutação. As mutações do Vuex são muito parecidas com eventos: cada mutação possui uma string **type** (ou __tipo__ ) e um **handler** (ou __manipulador__). A função handler é onde acontece as modificações de estado de fato, e vai receber o estado como seu primeiro argumento.
