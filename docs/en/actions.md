...
This may look dumb at first sight: if we want to increment the count, why don't we just call `store.commit('increment')` directly? Remember that **mutations must be synchronous**? Actions need not be. We can perform **asynchronous** operations inside an action:
