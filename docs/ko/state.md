...
Vuex는 **단일 상태 트리** 를 사용합니다. 즉, 이 단일 객체는 모든 응용 프로그램 수준의 상태를 포함하며 "원본 소스" 역할을 합니다. 이는 각 응용 프로그램마다 하나의 저장소만 갖게 된다는 것을 의미합니다. 단일 상태 트리를 사용하면 특정 상태를 쉽게 찾을 수 있으므로 디버깅을 위해 현재 앱 상태의 스냅 샷을 쉽게 가져올 수 있습니다.
