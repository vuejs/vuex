...
Разбираясь с первой проблемой, вам придётся передавать одни и те же данные в несколько глубоко вложенных компонентов. Это часто сложно и неприятно, а для соседних компонентов такое и вовсе не сработает. Решая вторую проблему, мы нередко прибегаем к таким средствам как обращение напрямую к родителям/потомкам компонента, или синхронизаций изменений с другими местами в приложении с помощью событий. Оба подхода хрупки и быстро приводят к появлению кода, который невозможно поддерживать.
